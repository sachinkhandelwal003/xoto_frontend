      ...
      <div className="group relative flex flex-col bg-white rounded-lg shadow-md overflow-hidden w-full max-w-sm transition-transform duration-300 hover:scale-105 hover:shadow-xl">
        ...
        <div className="p-4">
          <p className="text-xl font-bold text-[#1a237e]">
            {deal.price} <span className="text-gray-500 text-sm font-normal">/{deal.monthly ? 'month' : ''}</span>
          </p>
          <h4 className="text-lg font-semibold text-gray-900 mt-1">{deal.title}</h4>
          ...
          <div className="border-t border-gray-100 pt-3 mt-3 flex justify-between space-x-2 text-gray-500 text-sm">
            <div className="flex items-center"><span className="mr-1">{deal.beds}</span>Beds</div>
            <div className="flex items-center"><span className="mr-1">{deal.bathrooms}</span>Baths</div>
            <div className="flex items-center"><span className="mr-1">{deal.area}</span>Area</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="font-sans min-h-screen relative p-8">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <img 
          src={cardsBg} alt="City Skyline" 
          className="w-full h-full object-cover opacity-60" 
          style={{ filter: 'grayscale(10%) brightness(0.6)' }}
        />
        <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-blue-700 to-transparent z-0"></div>
      </div>
