...
function PrivateRoute({ children, allowedRoles }) {
  const user = { role: { code: "1" } }; // NOTE: This is hardcoded, ensure logic matches your Auth context
  const loading = false;
  const location = useLocation();
...
function LayoutWrapper({ children }) {
  ...
  const showEcommerceNavbar =
    location.pathname === "/ecommerce" || location.pathname === "/ecommerce/cart";
